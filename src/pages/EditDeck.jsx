...
function EditDeck() {
    ...
    return (
        <div className="min-h-screen text-white p-8 flex flex-col items-center gap-8 relative bg-cover bg-center bg-fixed"
            ...
            <div className="relative z-10 w-full flex flex-col items-center gap-8">
                ...
                {deck.status === 'published' && (
                    <div className="w-full max-w-lg bg-black/60 rounded-lg p-3 font-mono text-xs break-all">
                ...
                <div className="w-full max-w-lg flex flex-col gap-3">
                    ...
                    {deck.cards.map((card) =>
                        editingCardId === card.id ? (
                            <div key={card.id} className="bg-black/60 rounded-lg p-3 flex flex-col gap-2 border border-amber-400/20">
                                <input
                                    value={editForm.name}
                                ...
                                <textarea
                                    ...
                                    rows={6}
                                    className="bg-black/70 rounded-lg px-3 py-2 text-lg outline-none focus:ring-2 focus:ring-amber-400 resize-none"
                                />
                                <input
                            ...
                            <div key={card.id} className="bg-black/60 rounded-lg p-3 flex items-center justify-between border border-amber-400/20">
                                <div>
                                    <p className="font-semibold text-lg text-emerald-800">{card.name}</p>
                ...
                <form onSubmit={handleAddCard} className="w-full max-w-lg flex flex-col gap-3 bg-black/60 rounded-lg p-4 border border-amber-400/20">
                    <h2 className="text-xl font-semibold text-white/80">Add a card</h2>
                    <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Card name" className="bg-black/70 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-amber-400" />
                    <input value={quote} onChange={(e) => setQuote(e.target.value)} placeholder="Quote (optional)" className="bg-black/70 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-amber-400" />
